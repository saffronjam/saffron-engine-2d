	...
	Singleton(this)
{
	...
	GuiImpl::SFML::Init(win.NativeWindow(), true);
	win.AnyEvent.Subscribe([](const sf::Event& event)
	{
	...
	MenuBar::AddMenu({"Gui", SE_EV_ACTION(Gui::OnRenderMenuBar)}, 0);
...
{
	return Property(name, value, "%.3f", min, max, step, flags, fn);
}
...
void Gui::SetFontSize(int size)
{
	...
	{
		Log::CoreError("Failed to fetch appropriate font");
		return;
	}
