...
#include "saffron/lighting/lightning_mgr.h"
...
{
	if (!_shadowMapShader.loadFromFile("Resources/Shaders/vertexShader.vert", "assets/shaders/shadowMap.frag"))
	{
		Log::CoreError("Failed to load shadow map shader");
	}
	if (!_renderLightsShader.loadFromFile("Resources/Shaders/vertexShader.vert", "assets/shaders/renderLights.frag"))
	{
		Log::CoreError("Failed to load render lights shader");
	}
	const auto& size = App::Instance().Window().Size();
	if (!_collectiveLightMap.create(size.x, size.y))
	{
		Log::CoreError("Failed to create collective light map {0}x{1}", size.x, size.y);
	}
}
...
void LightningMgr::AddOccluder(const sf::Drawable& drawable, sf::FloatRect boundingBox, sf::RenderStates renderStates)
{
...
{
	for (auto& renderLight : _renderLights) renderLight.occluders->clear(sf::Color(0, 0, 0, 0));
}
...
void LightningMgr::RemoveLight(const Light::Ptr& light)
{
	...
	if (findRes != _renderLights.end()) _renderLights.erase(findRes);
}
