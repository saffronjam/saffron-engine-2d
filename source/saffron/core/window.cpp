	...
	_nativeWindow(_videomode, std::move(title), _style, sf::ContextSettings(0u, 0u, 0u, 1u, 4u, 0u))
{
	...
	if (!gladLoadGL())
	{
		Log::CoreError("Failed to initialize OpenGL context");
	}
...
void Window::DrawPoint(const sf::Vector2f& position, sf::Color color, float radius)
{
	...
	Draw(circle, sf::RenderStates::Default);
}
...
void Window::DrawRect(const sf::FloatRect& rect, sf::Color fillColor, bool outlined, sf::Color outlineColor)
{
	...
	Draw(rectShape, sf::RenderStates::Default);
}
...
void Window::DrawLine(const sf::Vector2f& first, const sf::Vector2f& second, sf::Color color)
{
	...
	Draw(line, sf::RenderStates::Default);
}
...
{
	const auto result = TrySetIcon(path);
	if (!result)
	{
...
auto Window::TrySetIcon(const std::filesystem::path& path) -> Status
{
	auto icon = ImageStore::TryGet(path, false);
	if (!icon)
	{
