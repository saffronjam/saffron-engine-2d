...
#include "saffron/core/date_time.h"
...
auto DateTime::IsBefore(const DateTime& rhs) const -> bool
{
...
auto DateTime::IsAfter(const DateTime& rhs) const -> bool
{
	const auto& lDate = _date;
	const auto& rDate = rhs._date;
...
auto DateTime::WeekdayString(bool abbreviation) const -> std::string
{
	if (abbreviation)
	{
		return ValueToWeekdayShortString(Weekday());
	}
	return ValueToWeekdayString(Weekday());
}
...
auto DateTime::MonthString(bool abbreviation) const -> std::string
{
	if (abbreviation)
	{
		return ValueToMonthShortString(Month());
	}
	return ValueToMonthString(Month());
}
