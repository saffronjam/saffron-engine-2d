...
protected:
	void BuildBase(const sf::Vector2f& position, float radius, sf::Color color,
	               const sf::Vector2f& direction, float theta);
...
protected:
	void AddStaticOccluder(const sf::Drawable& drawable, const sf::RenderStates& renderStates);
