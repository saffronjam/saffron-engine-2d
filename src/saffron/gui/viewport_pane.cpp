...
void ViewportPane::OnGuiRender(bool* open, UUID uuid)
{
	...
	{
		Gui::Image(_target->Texture(), {viewportSize.x, viewportSize.y}, {0.0f, 1.0f}, {1.0f, 0.0f},
		           sf::Color::White, sf::Color::Transparent);
	}
	else
	{
		Gui::Image(_fallbackTexture, {viewportSize.x, viewportSize.y}, sf::FloatRect{0.0f, 0.0f, 1.0f, 1.0f}, sf::Color::White, sf::Color::Transparent);
	}

	auto borderColor = IM_COL32(255, 140, 0, 80);
	if (_focused)
	{
		borderColor = IM_COL32(255, 140, 0, 180);
	}
	ImGui::GetWindowDrawList()->AddRect(ImVec2(_topLeft.x, tl.y), ImVec2(br.x, br.y),
	                                    borderColor, 0.0f, ImDrawCornerFlags_All, 4);
