...
#include "saffron/gui/gui_impl.h"
...
sf::Texture* s_fontTexture = nullptr; // owning pointer to internal font atlas which is used if user
...
auto toImColor(sf::Color c) -> ImColor;
auto getTopLeftAbsolute(const sf::FloatRect& rect) -> ImVec2;
auto getDownRightAbsolute(const sf::FloatRect& rect) -> ImVec2;

auto convertGLTextureHandleToImTextureID(GLuint glTextureHandle) -> ImTextureID;
auto convertImTextureIDToGLTextureHandle(ImTextureID textureID) -> GLuint;

void RenderDrawLists(ImDrawData* draw_data); // rendering callback function prototype

// Implementation of ImageButton overload
auto imageButtonImpl(const sf::Texture& texture, const sf::FloatRect& textureRect, const sf::Vector2f& size,
                     int framePadding, const sf::Color& bgColor, const sf::Color& tintColor) -> bool;

// Default mapping is XInput gamepad mapping
void initDefaultJoystickMapping();

// Returns first id of connected joystick
auto getConnectedJoystickId() -> unsigned int;
...
auto getClipboadText(void* userData) -> const char*;
...
void ProcessEvent(const sf::Event& event)
{
	if (s_windowHasFocus)
	{
		...
		switch (event.type)
		{
		case sf::Event::MouseMoved: s_mouseMoved = true;
			break;
		...
		break;
		case sf::Event::MouseWheelScrolled: if (event.mouseWheelScroll.wheel == sf::Mouse::VerticalWheel || (event.
				mouseWheelScroll.wheel == sf::Mouse::HorizontalWheel && io.KeyShift))
			{
		...
		case sf::Event::KeyReleased: io.KeysDown[event.key.code] = (event.type == sf::Event::KeyPressed);
			break;
		case sf::Event::TextEntered:
			...
			break;
		case sf::Event::JoystickConnected: if (s_joystickId == NULL_JOYSTICK_ID)
			{
				s_joystickId = event.joystickConnect.joystickId;
			}
			break;
		case sf::Event::JoystickDisconnected: if (s_joystickId == event.joystickConnect.joystickId)
			{
	...
	switch (event.type)
	{
	case sf::Event::LostFocus: s_windowHasFocus = false;
		break;
	case sf::Event::GainedFocus: s_windowHasFocus = true;
		break;
	default: break;
	}
}
...
void Update(const sf::Vector2i& mousePos, const sf::Vector2f& displaySize, sf::Time dt)
{
	...
	if (s_windowHasFocus)
	{
		...
		{
			io.MouseDown[i] = s_touchDown[i] || sf::Touch::isDown(i) || s_mousePressed[i] || sf::Mouse::isButtonPressed(
...
void UpdateFontTexture()
{
	...
	{
		saffron::Log::CoreError("Failed to create ImGui font texture {0}x{1}", width, height);
		return;
	}
...
auto GetFontTexture() -> sf::Texture& { return *s_fontTexture; }
...
{
	if (action >= ImGuiNavInput_COUNT || joystickButton >= sf::Joystick::ButtonCount)
	{
		return;
	}
...
void Image(const sf::Texture& texture, const sf::FloatRect& uvRect, const sf::Color& tintColor,
           const sf::Color& borderColor)
{
	Image(texture, saffron::GenUtils::ConvertTo<float>(texture.getSize()), uvRect, tintColor, borderColor);
}
           ...
           const sf::Color& tintColor, const sf::Color& borderColor)
{
	...
	const ImTextureID textureID = convertGLTextureHandleToImTextureID(texture.getNativeHandle());
	ImGui::Image(textureID, ImVec2(size.x, size.y), imUv0, imUv1, toImColor(tintColor), toImColor(borderColor));
}
...
auto ImageButton(const sf::Texture& texture, const int framePadding, const sf::Color& bgColor,
...
auto ImageButton(const sf::Texture& texture, const sf::Vector2f& size, const int framePadding, const sf::Color& bgColor,
                 const sf::Color& tintColor) -> bool
{
	const sf::Vector2f textureSize = static_cast<sf::Vector2f>(texture.getSize());
	return imageButtonImpl(texture, sf::FloatRect(0.f, 0.f, textureSize.x, textureSize.y), size, framePadding, bgColor,
	                       tintColor);
}

auto ImageButton(const sf::Sprite& sprite, const int framePadding, const sf::Color& bgColor,
                 const sf::Color& tintColor) -> bool
{
...
auto ImageButton(const sf::Sprite& sprite, const sf::Vector2f& size, const int framePadding, const sf::Color& bgColor,
                 const sf::Color& tintColor) -> bool
{
	const sf::Texture* texturePtr = sprite.getTexture();
	if (!texturePtr)
	...
	return imageButtonImpl(*texturePtr, static_cast<sf::FloatRect>(sprite.getTextureRect()), size, framePadding,
	                       bgColor, tintColor);
}
...
{
	ImDrawList* draw_list = ImGui::GetWindowDrawList();
	draw_list->AddRectFilled(getTopLeftAbsolute(rect), getDownRightAbsolute(rect), toImColor(color), rounding,
	                         rounding_corners);
}
...
auto convertGLTextureHandleToImTextureID(GLuint glTextureHandle) -> ImTextureID
{
	ImTextureID textureID = static_cast<ImTextureID>(nullptr);
...
void RenderDrawLists(ImDrawData* draw_data)
{
	...
	ImGuiIO& io = ImGui::GetIO();
	if (io.Fonts->TexID == (ImTextureID)NULL)
	{
		return;
	}
...
auto imageButtonImpl(const sf::Texture& texture, const sf::FloatRect& textureRect, const sf::Vector2f& size,
                     const int framePadding, const sf::Color& bgColor, const sf::Color& tintColor) -> bool
{
	const sf::Vector2f textureSize = static_cast<sf::Vector2f>(texture.getSize());
...
auto getConnectedJoystickId() -> unsigned int
{
	for (unsigned int i = 0; i < static_cast<unsigned>(sf::Joystick::Count); ++i)
	{
...
void initDefaultJoystickMapping()
{
	...
	GuiImpl::SFML::SetDPadXAxis(sf::Joystick::PovX, false);
	// D-pad Y axis is inverted on Windows
#ifdef _WIN32
	GuiImpl::SFML::SetDPadYAxis(sf::Joystick::PovY, true);
#else
	GuiImpl::SFML::SetDPadYAxis(sf::Joystick::PovY, false);
#endif

	GuiImpl::SFML::SetLStickXAxis(sf::Joystick::X, false);
	GuiImpl::SFML::SetLStickYAxis(sf::Joystick::Y, false);
...
void updateJoystickActionState(ImGuiIO& io, ImGuiNavInput_ action)
{
	...
	io.NavInputs[action] = 0.0f;
	if (isPressed)
	{
		io.NavInputs[action] = 1.0f;
	}
}
...
void updateJoystickDPadState(ImGuiIO& io)
{
	...
	if (dpadXPos < -s_dPadInfo.threshold)
	{
	...
	if (dpadYPos < -s_dPadInfo.threshold)
	{
