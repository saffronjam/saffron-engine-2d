...
auto Voronoi::Polygon::HasSameId(const Polygon& other) const -> bool { return _uuid.IsEqualTo(other._uuid); }
...
{
	const auto result = std::ranges::find_if(_polygons, [&polygon](const Polygon& candidate)
	{
		return candidate.HasSameId(polygon);
	});
...
void Voronoi::Relax(int iterations)
{
	if (!_diagram.has_value())
	{
		Log::CoreError("Voronoi was not created. Diagram was not instantiated");
		return;
	}
...
auto Voronoi::PolygonAt(const sf::Vector2f& position) -> Voronoi::Polygon&
{
	...
	if (closest == nullptr)
	{
		Log::CoreError("Failed to find a Voronoi polygon for position");
...
{
	if (_boundingBox.width <= 0.0f || _boundingBox.height <= 0.0f)
	{
		Log::CoreError("Invalid Voronoi bounding box dimensions");
		return;
	}
	if (_points.empty())
	{
		Log::CoreError("No Voronoi points available");
		return;
	}
